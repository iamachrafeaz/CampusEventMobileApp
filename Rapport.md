...
### Mini-projet React Native · Université Abdelmalek Essaâdi — Département Informatique
...
La navigation est gérée par **Expo Router** (file-based routing). Au lancement, l'application détecte l'état de session et redirige vers l'écran de connexion ou directement vers l'interface correspondant au rôle. Deux arbres de navigation distincts existent : l'espace Admin (Stack) et l'espace Étudiant (Bottom Tabs avec 4 onglets).
...
La validation des identifiants est effectuée côté client par le `loginService`, qui compare les valeurs saisies aux credentials statiques. En cas de succès, l'état global est mis à jour via **Zustand** (`useAuthStore`) avec le rôle détecté et l'identifiant utilisateur, puis la navigation redirige vers l'espace approprié.

```typescript
// store/useAuthStore.ts (Zustand)
login: (userType, user) => set({ isLoggedIn: true, userType, user })
logout: () => set({ isLoggedIn: false, userType: null, user: null })
```
...
**`events`** — catalogue principal des événements  
Champs : `id` (UUID), `title`, `description`, `category`, `startDateTime`, `endDateTime`, `locationName`, `locationAddress`, `organizerName`, `capacity`, `registeredCount`, `imageUrl`, `tags` (JSON sérialisé), `createdAt`.

**`registrations`** — inscriptions des étudiants  
Champs : `id`, `eventId` (FK → events ON DELETE CASCADE), `userId`, `createdAt`, `status` (`confirmed` | `cancelled`).

**`favorites`** — favoris par utilisateur  
Clé primaire composite (`eventId`, `userId`), FK → events ON DELETE CASCADE.

**`llm_results`** — cache des réponses IA  
Champs : `id`, `eventId` (nullable pour les résultats transversaux), `userId`, `type` (`search` | `recommendation` | `planning` | `qa`), `inputText`, `outputText`, `createdAt`.

### Contraintes d'intégrité

Les clés étrangères avec `ON DELETE CASCADE` garantissent que la suppression d'un événement par l'administrateur entraîne automatiquement la suppression des inscriptions et favoris associés — règle métier critique respectée au niveau SQL.

Les PRAGMA `foreign_keys = ON` sont activés à l'initialisation pour forcer l'application des contraintes.
...
Message utilisateur
        ↓
  [base.prompt] → classification : search | recommend | plan | qa
        ↓
  Extraction des données SQLite selon le type
        ↓
  [prompt spécialisé + données JSON] → LLM Groq
        ↓
  Composant de rendu adapté au type de réponse
```
...
**Limite de contexte :** ~200 caractères (message utilisateur uniquement).
...
**Limite de contexte :** 4 000 à 8 000 caractères selon la taille du catalogue.
...
**Limite de contexte :** Environ 6 000 caractères (historique + catalogue filtré).
...
**Limite de contexte :** 5 000 à 7 000 caractères (contraintes + événements filtrés sur la semaine).
...
|------------|---------|------|
| Expo | ~54.0 | Plateforme de développement multiplateforme |
| React Native | 0.81.5 | Framework UI mobile |
| TypeScript | ~5.9 | Typage statique |
| Expo Router | ~6.0 | Navigation file-based |
| expo-sqlite | ~16.0 | Persistance locale SQLite |
| Zustand | ^5.0 | Gestion d'état global léger |
| groq-sdk | ^1.1 | Intégration LLM via API Groq |
...
Le fournisseur retenu est **Groq** avec le modèle `llama-3.3-70b-versatile` (ou équivalent selon disponibilité), choisi pour sa rapidité d'inférence particulièrement adaptée à une expérience conversationnelle mobile.
...
**Architecture locale uniquement :** Les deux profils (Admin et Étudiant) partagent la même base SQLite sur le même appareil. Dans un déploiement réel, un backend avec authentification et base de données partagée serait nécessaire.

**Pas de synchronisation temps réel :** Si l'admin modifie un événement pendant qu'un étudiant consulte le catalogue, l'étudiant ne verra les changements qu'au rechargement.

**Limite de contexte LLM :** Pour un catalogue de plusieurs centaines d'événements, la sérialisation JSON complète peut dépasser la fenêtre de contexte du modèle. Une stratégie de filtrage préalable côté SQLite (pré-filtre par date, catégorie) atténue ce problème mais ne l'élimine pas totalement.

**Cache IA simplifié :** Le cache actuel (`llm_results`) stocke les résultats par `inputText` exact. Une requête légèrement reformulée ne trouvera pas le cache et déclenchera un nouvel appel API.

### Perspectives d'évolution

- **Backend réel** : API REST avec authentification JWT, base de données partagée, notifications push serveur.
- **Profil étudiant enrichi** : Filière, année, intérêts déclarés pour améliorer la pertinence des recommandations (déjà partiellement implémenté via `AsyncStorage`).
- **Rappels locaux** : Notifications push avant les événements inscrits via `expo-notifications`.
- **Cache sémantique** : Comparaison par embeddings des requêtes pour trouver des résultats en cache même avec une reformulation différente.
- **Résumé hebdomadaire automatique** : Digest IA généré chaque semaine pour présenter les événements à venir les plus pertinents.

---

*Rapport rédigé dans le cadre du Mini-projet React Native — CampusEvents AI*  
*Université Abdelmalek Essaâdi — Département Informatique · Mai 2026*
